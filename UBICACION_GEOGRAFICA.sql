/* Autor: ERIN SIEZAR
   FECHA: 31/5/2015*/
CREATE TABLE UBICACION_GEOGRAFICA(
	UBICACION_GEOGRAFICA_ID INT(8) NOT NULL AUTO_INCREMENT PRIMARY KEY,
    DESCRIPCION VARCHAR(100) unique,
    FEC_CREACION DATE,
	USUARIO_CREACION VARCHAR(10),
	FEC_ULTIMA_MODIFICACION DATE,
	USUARIO_ULTIMA_MODIFICACION VARCHAR(10)
);

DELIMITER $$
CREATE TRIGGER BEFORE_INSERT_UBI_GEO
  BEFORE INSERT ON UBICACION_GEOGRAFICA FOR EACH ROW
BEGIN
	SET NEW.USUARIO_CREACION:= current_user();
    SET NEW. FEC_CREACION:= curdate();
END$$

DELIMITER $$
CREATE TRIGGER BEFORE_UPDATE_UBI_GEO
  BEFORE UPDATE ON PROGRA_2.UBICACION_GEOGRAFICA FOR EACH ROW
BEGIN
	SET NEW.USUARIO_ULTIMA_MODIFICACION:= current_user();
	SET NEW.FEC_ULTIMA_MODIFICACION:= curdate();
END$$ 
